...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Check settings
    const data = await chrome.storage.local.get(['apiKey', 'userResume', 'provider', 'ollamaModel', 'ollamaUrl']);

    // Basic validation: need resume. API key needed only if not using Ollama.
    if (!data.userResume) {
...
async function handleGenerate() {
    ...
    try {
        ...
        try {
            ...
            const response = await chrome.tabs.sendMessage(tab.id, { action: 'getJobDescription' });
        ...
        if (!jobDescription || jobDescription.length < 50) {
            console.error('Job description too short or empty:', jobDescription?.length);
            throw new Error('Could not extract job description. Please make sure you are on a job page or Refresh the page.');
        }
        ...
        if (genResume) {
            ...
            const resumeContent = await generateWithAI('resume', jobDescription, currentUserResume, currentConfig.provider, currentConfig);
        ...
        if (genCoverLetter) {
            ...
            const clContent = await generateWithAI('cover-letter', jobDescription, currentUserResume, currentConfig.provider, currentConfig);
        ...
        if (genEmail) {
            ...
            const emailContent = await generateWithAI('email', jobDescription, currentUserResume, currentConfig.provider, currentConfig);
...
function exportToDOCX() {
    ...
    if (navigator.msSaveOrOpenBlob) {
        ...
        downloadLink.download = `${activeTab.replace(/\s+/g, '_')}_ResumeEntropy.doc`;
        downloadLink.click();
    }
